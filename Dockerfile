...
FROM alpine/git:latest AS wpcontent
ARG SECONDARY_REPO_URL
ARG SECONDARY_REPO_BRANCH
ARG WP_CONTENT_PATH
RUN mkdir -p /tmp/wpassets/${WP_CONTENT_PATH} \
...
COPY --from=wpcontent /tmp/wpassets/${WP_CONTENT_PATH}/ /var/www/html/wp-content/